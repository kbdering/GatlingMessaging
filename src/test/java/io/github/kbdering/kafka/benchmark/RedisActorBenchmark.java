...
public class RedisActorBenchmark {
    ...
    public static void main(String[] args) throws InterruptedException {
        // Start Redis
        try (GenericContainer<?> redis = new GenericContainer<>("redis:7.2.4").withExposedPorts(6379)) {
            redis.start();

            // Configure JedisPool
            JedisPoolConfig poolConfig = new JedisPoolConfig();
            poolConfig.setMaxTotal(NUM_PRODUCERS + NUM_CONSUMERS + 50);
            poolConfig.setMaxIdle(NUM_PRODUCERS + NUM_CONSUMERS + 50);
            poolConfig.setMinIdle(50);
            JedisPool jedisPool = new JedisPool(poolConfig, redis.getHost(), redis.getMappedPort(6379));

            RedisRequestStore store = new RedisRequestStore(jedisPool);
            ...
            ActorSystem system = ActorSystem.create("RedisBenchmarkSystem", config);
    ...
    private static void runBenchmark(ActorSystem system, RedisRequestStore store, int batchSize)
    ...
    static class ProducerActor extends AbstractActor {
        private final RedisRequestStore store;
        private final CountDownLatch latch;
        private final long endTime;
        private final Queue<String> queue;
        private final AtomicLong producedOps;

        public static Props props(RedisRequestStore store, CountDownLatch latch, long endTime, Queue<String> queue,
                AtomicLong producedOps) {
            return Props.create(ProducerActor.class,
                    () -> new ProducerActor(store, latch, endTime, queue, producedOps));
        }

        public ProducerActor(RedisRequestStore store, CountDownLatch latch, long endTime, Queue<String> queue,
                AtomicLong producedOps) {
            this.store = store;
            this.latch = latch;
    ...
    static class ConsumerActor extends AbstractActor {
        ...
        public static Props props(RedisRequestStore store, CountDownLatch latch, long endTime, int batchSize,
        ...
        public ConsumerActor(RedisRequestStore store, CountDownLatch latch, long endTime, int batchSize,
                Queue<String> queue, AtomicLong consumedOps) {
            this.store = store;
            this.latch = latch;
            this.endTime = endTime;
        ...
        private void processBatch() {
            ...
            if (!batch.isEmpty()) {
                store.processBatchedRecords(batch, new io.github.kbdering.kafka.cache.BatchProcessor() {
                    @Override
