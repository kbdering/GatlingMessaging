...
public class UninitializedAvroSimulation extends Simulation {
...

    {
        ...
        ScenarioBuilder scn = scenario("Uninitialized Avro Test")
                .exec(
                        KafkaDsl.kafka("Send Empty Record")
                                .send()
                                .topic("uninitialized-avro-topic")
                                .key(session -> UUID.randomUUID().toString())
                                .value(session -> {
                                    // We create the record but NEVER call .put()
                                    // Since 'name' and 'age' are mandatory, this should fail during serialization.
