...
class KafkaAvroScalaSimulation extends Simulation {
  ...
  val kafkaProtocol = kafka()
    ...
    .groupId("gatling-scala-avro-" + UUID.randomUUID().toString.substring(0, 8))
    .numProducers(1)
    .numConsumers(1)
  ...
  // Demonstrates session variable propagation using Java Session API.
  // Session attributes set in .exec { ... } are accessible in the
  // subsequent .exec(kafka(...).asScala()) via session.getString() / session.get()
  val scn = scenario("Send Avro Order")
    .exec { scalaSession =>
      ...
      // Return the updated session — Scala Session.set() returns a new session
      scalaSession
    ...
    .exec(
      kafka("Send Avro Message")
        .send()
        .topic(AVRO_TOPIC)
        // Java Session API: session.getString(), session.get()
        .key(session => session.getString("myCorrelationId"))
        .value(session => session.get("myAvroPayload"))
        .asAvro()
        .asScala()
    )
    .exec(
      consume("Consume Avro Message", AVRO_TOPIC)
        ...
        .check(Checks.field(classOf[org.apache.avro.generic.GenericRecord])
          ...
          .satisfies(name => name.equals("John Doe")))
        .asScala()
    )
    .pause(5)
